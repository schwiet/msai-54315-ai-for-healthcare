...
Our final patient vector consisted of our $426$-dimension structured features plus the $768$-dimensional vector from the clinical BERT embeddings. Our hybrid patient representation had $41{,}117$ patients, each with a combined vector dimension of $1{,}194$.


\subsection{Retrieval Strategy}
With the patient vectors in their multi-modal format, we are able to perform patient-to-patient similarity searches. Given one patient's vector, we could find the $k$ nearest neighbors in the patient space. We used the \texttt{sklearn} library \cite{scikitlearn_neighbors_docs} to perform the nearest neighbor search with the cosine distance metric.
...
It became clear that our retrieval strategy would need to be made modular, so it could handle choosing an appropriate search strategy based on the query. We decided to incorporate the LLM as a pre-step to the retrieval process, to handle the query in a more flexible way. We effectively used the LLM as a routing layer, to guide the retrieval process based on the intent of the query. 
...
Once the router has identified the search space most appropriate to address the query's intent, we embed the query and search the filtered space using only the embeddings from the discharge summaries. These queries would not be able to utilize the multi-modal patient representations, but by reducing their search space we attempted to improve the chances of finding relevant result.
...
Because we used the DGX Spark, we had access to a large amount of memory and decided to opt for precision of performance by not using 4-bit quantization. Additionally, this environment allowed us to use larger models, such as the OpenBioLLM-70B model \cite{hf_llama3_openbiollm_70b}, which a 70 billion parameter advanced open source language model designed specifically for the biomedical domain.
...
The resulting system has aspects that are promising, but also significant limitations. We believe that there are opportunities to improve the synthesis between the retrieval and generation stages to improve the quality of the responses.

We have provided excerpts from a response that is a representative example of the system's capabilities and limitations.
...
The application can currently handle routing to the correct retrieval strategy (``correct'', as defined by the authors) and it succeeds at performing filtered retrieval appropriately, based on the extracted features. This has the desired effect of reducing the search space to only patients that match the query parameters, though the feature extraction is not perfect. As demonstrated in this example, the application does provide an explanation for how well the provided patients match the query parameters, but this is where we have identified opportunities to improve the accuracy and usefulness of the output.

\subsection{Future Work}

\subsubsection{Improving the Feature Extraction}
The feature extraction with the OpenBioLLM-70B model is noticeably better than with Mistral 7B, but it is sometimes too generous in the features that it extracts (e.g. diagnosis of ``history of smoking''). We have already made improvements by making adjustments to the prompt, but there is still opportunity to make it more accurate.

\subsubsection{Relevant Details from Search Strategy missing from the Generation}
The example above illustrates scenarios where information related to the patient, such as diagnoses or age, is not present in the truncated discharge summary notes provided to the LLM for analysis. The next steps include investigating existing strategies for providing relevant information from notes without exceeding the model's context window. The current truncation approach is too simplistic. Alternatives proposed include generating a "patient profile" from the structured features to include with the notes in the prompt. This should ensure that none of the details extracted from the query and filtered-upon are missing from the generation step.

\section{Conclusion}

Clinicians routinely rely on experience and intuition to guide their decisions about patient care. Our aim in this project was to explore whether a RAG system could extend that mode of thinking to large-scale clinical datasets, such as MIMIC-III. By constructing a multi-modal patient representation, combining structured demographics and diagnoses with Clinical BERT embeddings of discharge summaries and embedding both data and queries, we built a system that attempts to surface clinically similar patients in response to natural language queries. The result is an early prototype that demonstrates both the promise and the complexity of making patient-level evidence discoverable by intuitive, conversational means.

Our findings highlight several successes. The hybrid retrieval routing strategy allowed us to adapt to diverse query types. Filtered search reduced irrelevant results enabling the generative model to produce clinical comparisons. The embedding workflow proved scalable and the multi-modal patient vectors provide a foundation for more sophisticated retrieval algorithms. The project also exposed key challenges. The modality gap limited the usefulness of combining structured and unstructured representations directly. The router, while functional, remains brittle to subtle variations in phrasing, limiting its usefulness for the intended use cases. And the generative step often lacked crucial clinical details because of context-window constraints, revealing a broader need for tighter integration between retrieval and synthesis.

Ultimately, this work shows that creating an intuitive patient-similarity search tool is feasible, but making it clinically useful requires further refinement. Future iterations will need more robust feature extraction, improved linkage between structured features and generative reasoning, and methods for preserving relevant patient details without overwhelming the model. Still, the system presented here offers a step toward making rich clinical datasets accessible in the same way clinicians access their own experience: by asking simple, natural questions and receiving explanations grounded in patient-level evidence.
